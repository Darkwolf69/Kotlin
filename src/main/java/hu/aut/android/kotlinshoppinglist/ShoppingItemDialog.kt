...
class ShoppingItemDialog : DialogFragment() {
    ...
    private fun handleItemDelete() {
        val itemToDelete = (arguments?.getSerializable(
            MainActivity.KEY_ITEM_TO_DELETE) as ShoppingItem).copy(name = etItem.text.toString(), price = etPrice.text.toString().toInt())

        shoppingItemHandler.shoppingItemDeleted(itemToDelete)
    }
}
