...
class MainActivity : AppCompatActivity(), ShoppingItemDialog.ShoppingItemHandler {
    companion object {
        ...
        val KEY_ITEM_TO_DELETE = "KEY_ITEM_TO_DELETE"
    }
    ...
    fun showDeleteItemDialog(itemToDelete: ShoppingItem) {
        val deleteDialog = ShoppingItemDialog()

        val bundle = Bundle()
        bundle.putSerializable(KEY_ITEM_TO_DELETE, itemToDelete)
        deleteDialog.arguments = bundle

        deleteDialog.show(supportFragmentManager, "TAG_ITEM_DELETE")
    }
    ...
    override fun shoppingItemDeleted(item: ShoppingItem) {
        thread {
            AppDatabase.getInstance(this@MainActivity).shoppingItemDao().deleteItem(item)
        }
    }
}
